...
export const runAudit = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<AuditReport> => {
    ...
    const hasMeta =
      /<meta[^>]+name=["']description["'][^>]+content=["'][^"']{100,160}["']/i.test(html)
    ...
    const onPageChecks: CheckItem[] = [
      ...
      check(
        ...
        hasMeta ? 'Meta description found within range.' : 'Meta description missing or outside range.'
      ),
      check(
        'op-tel',
    ...
    const hasNoindex =
      ...
      /X-Robots-Tag.*noindex/i.test(html)
    ...
    const technicalChecks: CheckItem[] = [
      check(
        ...
        'HTTPS enforced',
        isHttps ? 'pass' : 'fail',
        isHttps ? 'Site uses HTTPS.' : 'Site is not on HTTPS — critical security and ranking issue.'
      ),
      check(
        'tech-noindex',
    ...
    const healthSummary: AuditReport['healthSummary'] = [
      {
        ...
        priority: score(gbpChecks) === 'critical' ? 'P0' : score(gbpChecks) === 'needs-work' ? 'P1' : 'P2',
      },
      {
        area: 'On-page & schema',
        rating: score(onPageChecks),
        topIssue: topIssue(onPageChecks),
        priority: score(onPageChecks) === 'critical' ? 'P0' : score(onPageChecks) === 'needs-work' ? 'P1' : 'P2',
      },
      {
        area: 'Technical & indexing',
        rating: score(technicalChecks),
        topIssue: topIssue(technicalChecks),
        priority: score(technicalChecks) === 'critical' ? 'P0' : score(technicalChecks) === 'needs-work' ? 'P1' : 'P2',
      },
      {
        area: 'Rankings, content & reviews',
        rating: score(rankingsChecks),
        topIssue: topIssue(rankingsChecks),
        priority: score(rankingsChecks) === 'critical' ? 'P0' : score(rankingsChecks) === 'needs-work' ? 'P1' : 'P2',
      },
      {
        area: 'Citations & local authority',
        rating: score(citationsChecks),
        topIssue: topIssue(citationsChecks),
        priority: score(citationsChecks) === 'critical' ? 'P0' : score(citationsChecks) === 'needs-work' ? 'P1' : 'P2',
      },
      {
        area: 'LLM / AEO Visibility',
        rating: score(llmChecks),
        topIssue: topIssue(llmChecks),
        priority: score(llmChecks) === 'critical' ? 'P0' : score(llmChecks) === 'needs-work' ? 'P1' : 'P2',
      },
    ]

    // ── 13. Auto-generate diagnosis ───────────────────────────────────────
    const criticalAreas = healthSummary
      .filter((h) => h.rating === 'critical')
      .map((h) => h.area)
